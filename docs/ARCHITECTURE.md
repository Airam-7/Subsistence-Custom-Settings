...
`ProfileWriter` and `AtomicFileWriter` handle complete writes. Installation and Vanilla maintenance remain separate from profile editing. INI editing parses exact SCS commands, preserves unrelated bindings, detects conflicts, backs up files and writes the same five assignments in both input sections: plain `Bindings=` in `Engine.PlayerInput`, dotted `.Bindings=` in `ColdGame.ColdPlayerInput`. The parser retains each entry's operator. Installation detection requires two matching entries per profile with the correct operators; migration normalizes old SCS entries into these ten lines. A valid dotted child entry is not the earlier plain-array shadowing bug. Missing/duplicate required sections and ambiguous assignments block installation or automatic migration without writing.
