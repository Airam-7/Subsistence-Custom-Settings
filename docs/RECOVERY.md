...
- **Keyboard or mouse bindings are missing after an older SCS installation:** verify the game paths. SCS repairs an SCS-only `ColdGame.ColdPlayerInput` array by moving those SCS bindings to `Engine.PlayerInput`, preserving unrelated flags. If parsing or concurrent-change checks block repair, preserve the current INI and backups for inspection.
